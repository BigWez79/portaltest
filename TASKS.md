...
It is three files, not one: `scripts/import-staff.ts`, `scripts/staff-csv.ts`
and `tests/import-staff-csv.spec.ts` all go together, along with the
`import:staff` line in package.json and the layout entries in CLAUDE.md.
...
- **Tested the CSV parser** — `overnight/auto-2026-08-28-0300`. The parsing came
  out of `scripts/import-staff.ts` into `scripts/staff-csv.ts` — no filesystem,
  no Supabase, no argv — so it can be called from a test. 38 checks pin the
  mapping: `Title` and its aliases, `Yes` and the things that are not `Yes`,
  commas inside quotes, duplicates, a row with no address, a short row, and the
  "no active admin" warning. Warnings are returned rather than printed, which is
  what made them checkable. The script's own output is unchanged.
